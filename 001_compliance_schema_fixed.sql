...
-- Migration 001: Database Schema Modifications (MySQL Compatible)
-- Date: January 9, 2026
...
-- Add consent columns to users table (MySQL compatible syntax)
ALTER TABLE users 
ADD COLUMN behavioralConsent TINYINT(1) DEFAULT 0 COMMENT 'Layer 2 consent - behavioral profiling (default OFF)';

ALTER TABLE users 
ADD COLUMN marketingConsent TINYINT(1) DEFAULT 0 COMMENT 'Layer 3 consent - marketing notifications (default OFF)';

ALTER TABLE users 
ADD COLUMN marketingFrequency ENUM('off', 'low', 'medium', 'high') DEFAULT 'off' COMMENT 'Marketing notification frequency preference';

ALTER TABLE users 
ADD COLUMN consentUpdatedAt TIMESTAMP NULL COMMENT 'Last consent preferences update';

ALTER TABLE users 
ADD COLUMN profileTier ENUM('bronze', 'silver', 'gold', 'platinum', 'elite') DEFAULT 'bronze' COMMENT 'User profile tier based on data completion';

ALTER TABLE users 
ADD COLUMN governorate VARCHAR(100) NULL COMMENT 'Egyptian governorate (optional)';

ALTER TABLE users 
ADD COLUMN signupSource VARCHAR(100) NULL COMMENT 'How user found TaskKash (optional)';

ALTER TABLE users 
ADD COLUMN dataRetentionSchedule DATE NULL COMMENT 'Scheduled data deletion date';
...
('tier3', 'news_sources', 'Where do you get your news from?', 'من أين تحصل على أخبارك؟', 'multiple_choice',
 '["Social media", "News websites", "TV", "Radio", "Newspapers", "News apps", "Friends/Family", "I do not follow news"]', 9, 'content'),
