...
async function run() {
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      await pool.query('UPDATE users SET passwordHash = ?, role = "advertiser", isVerified = 1 WHERE email = ?', [hash, email]);
      console.log('User updated.');
    } else {
      console.log('Inserting new advertiser user...');
      await pool.query(
        'INSERT INTO users (email, passwordHash, role, registrationIp, isVerified) VALUES (?, ?, "advertiser", "127.0.0.1", 1)',
        [email, hash]
      );
